...
const COL = {
  ...
  // the authoritative signal that the operator actually intended to
  // send the patient invoice — we read it here because Josh's
  // coins-form-payment webhook prematurely sets Secondary Status =
  // "Sent to Patient" the moment a payment link gets generated,
  // which is BEFORE our operator clicks Send Invoice. Without this
...
export function mapMondayItemToSecClaim(item: MondayItem): SecClaim {
  ...
  return {
    ...
    // from one that just had a pay link auto-generated by Josh's
    // coins-form-payment webhook.
    sendInvoiceTriggered: (() => {
      // True for any non-empty trigger label. Today the column carries
      // either "Sent" (initial Send Invoice) or "Follow-up" (Send
      // Follow-Up resends). Both states mean the invoice has been
      // sent at least once, so the row belongs in Outstanding
      // Invoices either way — bucketOf reads this flag to route.
      const v = txt(item, COL.SEND_INVOICE_TRIGGER).trim();
