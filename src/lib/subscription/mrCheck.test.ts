...
describe("deriveMr", () => {
  ...
  it("blank MR, ordinary referral → light green", () => {
    const c = deriveMr({ mnExpiry: "", referralSource: "Patient", today: TODAY });
    expect(c.tone).toBe("ok");
    expect(c.light).toBe(true);
    expect(c.detail).toContain("No MR expiry on file");
  });
  ...
  it("blank MR, District Endochrine → red (hard stop)", () => {
    expect(deriveMr({ mnExpiry: "", referralSource: "District Endochrine", today: TODAY }).tone).toBe("bad");
  });
});
